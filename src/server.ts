...
app.use((_req, res, next) => {
  ...
  const csp = [
    ...
    "connect-src 'self' https://accounts.google.com https://*.google.com https://*.googleusercontent.com https://fonts.googleapis.com https://fonts.gstatic.com https://www.googletagmanager.com https://www.google-analytics.com https://connect.facebook.net https://graph.facebook.com https://www.facebook.com wss://cloud.brewos.io ws://localhost:* http://localhost:*",
...
app.use(
  "/api/logs",
  (req, _res, next) => {
    // Inject device relay for proxying requests to devices
    (req as unknown as Record<string, unknown>).deviceRelay = deviceRelay;
    next();
  },
  logsRouter
);
